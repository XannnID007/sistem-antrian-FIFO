...
@section('breadcrumb')
    ...
    <li class="flex items-center">
        <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
        <span class="text-gray-600">Edit Santri</span>
    </li>
@endsection
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow card-shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-xl font-semibold text-gray-900">Edit Data Santri</h2>
                <p class="text-sm text-gray-600 mt-1">Perbarui informasi data santri</p>
            </div>

            <form action="{{ route('santri.update', $santri) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <textarea name="alamat" id="alamat" rows="3" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('alamat', $santri->alamat) }}</textarea>
                            @error('alamat')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="tel" name="phone_wali" id="phone_wali" required
                                value="{{ old('phone_wali', $santri->phone_wali) }}"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('phone_wali')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="kamar" id="kamar"
                                value="{{ old('kamar', $santri->kamar) }}" placeholder="Contoh: A-101"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('kamar')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="tahun_masuk" id="tahun_masuk" required
                                value="{{ old('tahun_masuk', $santri->tahun_masuk) }}" min="2020"
                                max="{{ date('Y') + 1 }}"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('tahun_masuk')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1"
                            {{ old('is_active', $santri->is_active) ? 'checked' : '' }}
